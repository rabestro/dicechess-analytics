...
| `dice_sorted` | `VARCHAR(3)`. The three rolled dice (piece types), stored as strictly 3 sorted piece **letters cased by the side to move** — white `BPQ`, black `bpq`. A small legacy slice instead uses digits `1`–`6` (`1`=pawn … `6`=king, e.g. `"125"`) — see [Domain Conventions](/dicechess-analytics/domain-conventions). |
