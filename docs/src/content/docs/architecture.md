...
| `time_initial_sec` / `time_increment_sec` | Time control. Both are strictly in seconds — see [Domain Conventions](../domain-conventions). |
...
| `stake_currency` | In-game currency (`GOLD`), not an ISO code; an amount of `0` means a tournament game — see [Domain Conventions](../domain-conventions). |
