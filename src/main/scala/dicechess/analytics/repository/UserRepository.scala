...
object UserRepository:
  ...
  def updateLogin(id: UUID, lastLoginAt: OffsetDateTime, name: Option[String], pictureUrl: Option[String]): ConnectionIO[Int] =
    sql"""
      UPDATE users
  ...
  def updateActive(id: UUID, isActive: Boolean): ConnectionIO[Int] =
    sql"UPDATE users SET is_active = $isActive WHERE id = $id"
      .update
      .run
