import uuid
from sqlalchemy import Column, String, BigInteger, Integer, SmallInteger, Boolean, ForeignKey, DateTime, JSON, text
...
class Game(Base):
    ...
    white_player_id = Column(UUID(as_uuid=True), ForeignKey("players.id"), nullable=True, index=True)
    black_player_id = Column(UUID(as_uuid=True), ForeignKey("players.id"), nullable=True, index=True)
    mode = Column(String(10), nullable=False, server_default="classic")
    result = Column(SmallInteger, nullable=True)
    termination = Column(String(20), nullable=True) # Will use CheckConstraint or Enum in real DB, defined as String here
